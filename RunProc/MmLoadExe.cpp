...
void ShowErrorExe(const char *lpszText)
{
...
LPVOID MmRunExe(LPVOID lpData, DWORD dwSize)
{
	...
	// 获取镜像大小
	DWORD dwSizeOfImage = GetSizeOfImageExe(lpData);

	// 在进程中开辟一个可读、可写、可执行的内存块
	lpBaseAddress = ::VirtualAlloc(NULL, dwSizeOfImage, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	if (NULL == lpBaseAddress)
	{
		ShowErrorExe("VirtualAlloc");
		return NULL;
	}
	::RtlZeroMemory(lpBaseAddress, dwSizeOfImage);

	// 将内存PE数据按SectionAlignment大小对齐映射到进程内存中
	if (FALSE == MmMapFileExe(lpData, lpBaseAddress))
	{
		ShowErrorExe("MmMapFile");
		return NULL;
	}

	// 修改PE文件重定位表信息
	if (FALSE == DoRelocationTableExe(lpBaseAddress))
	{
		ShowErrorExe("DoRelocationTable");
		return NULL;
	}

	// 填写PE文件导入表信息
	if (FALSE == DoImportTableExe(lpBaseAddress))
	{
		ShowErrorExe("DoImportTable");
		return NULL;
	}
	...
	{
		ShowErrorExe("VirtualProtect");
		return NULL;
	}

	// 修改PE文件加载基址IMAGE_NT_HEADERS.OptionalHeader.ImageBase
	if (FALSE == SetImageBaseExe(lpBaseAddress))
	{
		ShowErrorExe("SetImageBase");
		return NULL;
	}
...
BOOL CallExeEntry(LPVOID lpBaseAddress)
{
	...
	// 跳转到入口点处执行
	__asm //默认只支持32位内联汇编，64位需要特殊配置平台工具集
	{
		mov eax, lpExeEntry
		jmp eax
	}
